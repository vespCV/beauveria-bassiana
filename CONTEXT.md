...
The applied delivery pathway for overview and Dutch reader prose: a selective trap admits the target insect; the insect picks up _B. bassiana_ conidia from a passage surface (1- or 2-way dispenser) and/or from sugar or protein bait; the insect is released and returns to the nest so inoculum can move by contact to nestmates (workers, larvae, pupae) before colony hygiene removes it. Kill-in-trap is not the success criterion. Preferred Dutch labels: catch-infect-release, selectieve val, dispenser, bait/baait.
...
The audit trail for full-text exclusions. Preferred agent-readable signal: a BibTeX export of **only** Zotero collection `99-excluded` at `temp/zotero/99-excluded.bib`. Every entry in that file is excluded. Sync via `@zotero-sync` into `input-phase2/99-excluded/records.csv` and `pdfs/99-excluded/`; keep screened-out lines in the literature overview draft; remove matching rows from the include CSV. Until that export exists (or a small excluded-only file replaces a full-library Downloads bib), read collection `99-excluded` from a copy of `/Users/md/Zotero/zotero.sqlite`, or fall back to `mabb phase2 99-excluded.md` plus `pdfs/99-excluded/`.
...
**Zotero → repo sync**:
One-way sync from local Zotero into the repo (`@zotero-sync`): collection membership drives `pdfs/` placement (including `99-excluded`); exclusions and annotations feed `input-phase2/99-excluded/`, `temp/zotero-annotations/`, and the literature overview draft. Overview **section** membership still follows the include CSV primary `screening_category` until an explicit include/exclude sync updates it. Prefer Obsidian citekey notes under `notes/beauveria-bassiana/zotero/` over regenerating annotation markdown.
...
**Screening category**:
A Phase 2 full-text bucket whose folder id matches `pdfs/{category}/` and `@file-pdfs` / `@zotero-sync`: systematic review ids `01-existing-products`, `02-efficacy-mechanics-delivery` (with `02a_efficacy`, `02b_strains_traits`, `02c_formulation_delivery`), `03-autodissemination-social`, `04-nontarget-ecotox`, `05-regulatory-policy`, `06-background-proxies`, plus supplemental lanes `00-key-papers` (Beauveria on *V. velutina* / other Vespidae), `07-vespideae-biocontrol` (Vespidae non-Bb microbes and chemical trap/bait proxies), and full-text exclusion lane `99-excluded`. Aligns with subquestion ids for `01`–`06` (parent `02` holds the three `02*` subquestions). Zotero collection names use hyphens for `02a-efficacy` style ids; repo `02*` folders use underscores.
...
**Temp scratch**:
Gitignored working output under `temp/`, including `temp/research/` (cited one-question findings, including optional subquestion `01-existing-products` registry/product lookups), `temp/handoff/` (session handoffs), `temp/overview/` (literature overview draft), `temp/zotero/` (DB copies, optional `99-excluded.bib`, sync reports), and `temp/zotero-annotations/` (annotation markdown when Obsidian citekey notes are absent). Agents may write here; promote keepers into notes or tracked docs only by explicit user action. `@research` is not the systematic search log.
...
**Input phase 2**:
Relevant-article list and related gate files under `input-phase2/` used to select and file PDFs for extraction (`relevant_articles_categorized.csv`, `pdfs_download_links.md`), plus full-text exclusions under `input-phase2/99-excluded/`.
